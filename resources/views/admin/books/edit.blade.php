<x-app-layout>
    <div class="p-4 sm:p-8 max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-3xl font-bold text-base-content leading-tight">Edit Data Buku</h2>
                <p class="text-sm text-base-content/60 mt-1">Perbarui informasi buku koleksi yang sudah ada.</p>
            </div>
        </div>
        ...
        <div class="bg-base-100 shadow-xl shadow-base-200/50 rounded-3xl border border-base-200 overflow-hidden">
            <form action="{{ route('admin.books.update', $book->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="space-y-6 bg-base-200/30 p-6 sm:p-8 rounded-2xl border border-base-200/50">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="form-control">
                                    ...
                                    <input name="author" value="{{ old('author', $book->author) }}"
                                        ...
                                        class="input input-bordered w-full rounded-xl focus:input-primary transition-colors"
                                        required>
                                </div>
                                <div class="form-control">
                                    <label class="label font-semibold">Penerbit <span
                            ...
                            <div class="form-control">
                                ...
                                <select name="category_id"
                                    class="select select-bordered w-full rounded-xl focus:select-primary transition-colors"
                                    required>
                        ...
                        <div class="space-y-6 bg-base-200/30 p-6 sm:p-8 rounded-2xl border border-base-200/50">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div class="form-control">
                                    ...
                                    <input name="isbn" value="{{ old('isbn', $book->isbn) }}"
                                        ...
                                        class="input input-bordered w-full rounded-xl font-mono text-sm focus:input-primary transition-colors"
                                        required>
                                    @error('isbn')
                                        <p class="text-error text-xs mt-1 font-medium">{{ $message }}</p>
                                    @enderror
                                ...
                                <div class="form-control">
                                    <label class="label font-semibold">Tahun Terbit <span
                                            class="text-error">*</span></label>
                                    <input name="year_published" type="number"
                                        value="{{ old('year_published', $book->year_published) }}" min="1900"
                                        max="{{ date('Y') }}"
                                        class="input input-bordered w-full rounded-xl focus:input-primary transition-colors"
                                        required>
                                </div>
                                <div class="form-control">
                                    <label class="label font-semibold">Stok Unit <span
                                            class="text-error">*</span></label>
                                    <input name="stock" type="number" value="{{ old('stock', $book->stock) }}"
                                        min="0"
                                        class="input input-bordered w-full rounded-xl focus:input-primary transition-colors"
                                        required>
                                </div>
                            ...
                            <div class="form-control">
                                ...
                                <textarea name="description" rows="4" placeholder="Tuliskan ringkasan atau sinopsis buku..."
                                    class="textarea textarea-bordered w-full rounded-xl leading-relaxed focus:textarea-primary transition-colors">{{ old('description', $book->description) }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="bg-base-200/30 p-6 sm:p-8 rounded-2xl border border-base-200/50">
                            <div class="flex flex-col sm:flex-row gap-6 items-start sm:items-center">
                                @if ($book->cover)
                                    <div
                                        class="shrink-0 w-24 h-32 rounded-xl overflow-hidden border-2 border-base-200 shadow-sm">
                                        <img src="{{ Str::startsWith($book->cover, 'http') ? $book->cover : asset('storage/' . $book->cover) }}"
                                            class="w-full h-full object-cover" alt="Cover Saat Ini">
                                    </div>
                                @endif
                                <div class="form-control flex-1 w-full">
                                    <label class="label font-semibold">Ganti Cover Baru (Opsional)</label>
                                    <input name="cover" type="file" accept="image/*"
                                        class="file-input file-input-bordered file-input-primary w-full rounded-xl">
                                    <label class="label">
                                        <span class="label-text-alt text-base-content/60 italic font-medium">Biarkan
                                            kosong jika tidak ingin mengubah cover.
                                            <br>
                                            Format yang didukung: JPG, PNG, WEBP (Maksimal 2MB)</span>
                                    </label>
                                </div>
                            </div>
                        </div>
                ...
                <!-- Action Buttons -->
                <div
                    class="bg-base-200/50 p-6 sm:px-10 border-t border-base-200 flex flex-col-reverse sm:flex-row gap-3 justify-end items-center">
                    <a href="{{ route('admin.books.index') }}"
                        class="btn btn-ghost rounded-xl w-full sm:w-auto hover:bg-base-300">Batal</a>
                    <button type="submit"
                        class="btn btn-primary rounded-xl px-10 shadow-lg shadow-primary/30 w-full sm:w-auto hover:-translate-y-0.5 transition-transform">
