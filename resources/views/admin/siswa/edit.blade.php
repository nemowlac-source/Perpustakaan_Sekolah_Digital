<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <div class="w-8 h-8 bg-warning rounded-lg flex items-center justify-center">
                <svg class="w-5 h-5 text-warning-content" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z">
                    </path>
                </svg>
            </div>
            <h2 class="text-2xl font-bold text-base-content">Edit Data Siswa</h2>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <nav class="flex items-center gap-2 text-sm text-base-content/60">
            ...
            <span class="text-base-content">Edit: {{ $siswa->name }}</span>
        </nav>

        {{-- Student Info Card --}}
        <div class="bg-base-100 rounded-xl border border-base-300 shadow-lg p-6">
            <div class="flex items-center gap-4">
                <div class="avatar placeholder">
                    <div class="bg-neutral text-neutral-content rounded-full w-16 h-16">
                        <span class="text-xl font-semibold">{{ strtoupper(substr($siswa->name, 0, 2)) }}</span>
                    </div>
                </div>
                <div class="flex-1">
                    <h3 class="text-lg font-semibold">{{ $siswa->name }}</h3>
                    <p class="text-base-content/60">NIS: {{ $siswa->nis }} • Username: {{ $siswa->username }}</p>
                    <div class="flex items-center gap-4 mt-2">
                        <span class="badge badge-sm {{ $siswa->is_active ? 'badge-success' : 'badge-warning' }}">
        ...
        <div class="bg-base-100 rounded-xl border border-base-300 shadow-lg overflow-hidden">
            <div class="bg-gradient-to-r from-warning to-warning-focus text-warning-content px-6 py-4">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-lg font-semibold">Edit Informasi Siswa</h3>
                </div>
                <p class="text-warning-content/80 text-sm mt-1">Perbarui data siswa sesuai kebutuhan</p>
            </div>
            ...
            <form action="{{ route('admin.siswa.update', $siswa) }}" method="POST" class="p-6 space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-base-200/50 rounded-lg p-4 border border-base-300">
                        <p class="text-sm text-base-content/70 mb-4">
                            Kosongkan field password jika tidak ingin mengubah password siswa saat ini.
                        </p>
                ...
                <div class="flex flex-col sm:flex-row gap-3 justify-end pt-2">
                    ...
                    <button type="submit" class="btn btn-warning order-1 sm:order-2">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
